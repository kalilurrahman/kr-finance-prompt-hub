...
## 2024-05-24 - Tighten Content Security Policy (CSP)
**Vulnerability:** Weak Content Security Policy in `index.html` allowing `unsafe-eval`.
**Learning:** Vite development builds might require `unsafe-eval` for some hot module replacement features, but production builds generally do not. Keeping `unsafe-eval` in the CSP increases the risk of successful XSS attacks if an injection vulnerability exists. Also, the CSP lacked `object-src 'none'` and `base-uri 'none'`, which are defense-in-depth best practices to prevent malicious plugin loading and relative URL hijacking.
**Prevention:** When configuring CSPs for static frontend applications built with Vite, ensure `unsafe-eval` is omitted from `script-src`, and explicitly define `object-src 'none'` and `base-uri 'none'` to maximize protection against client-side injection attacks.
